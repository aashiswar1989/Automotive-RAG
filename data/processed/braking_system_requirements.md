...
The one-box brake-by-wire architecture shall comprise the following primary elements: [web:18][web:21][web:28] Brake pedal module with redundant sensing.
...
The vehicle shall provide a parking brake function capable of holding the vehicle stationary under specified gradient and load conditions in both forward and reverse directions.[web:7]
...
The master cylinder and booster combination shall generate sufficient pressure to meet required braking performance under worst-case loading and temperature conditions.[web:5]
...
The hydraulic braking system shall maintain functional performance under environmental exposure to water, dust, road contaminants, and expected temperature extremes.[web:6]
...
The system shall continuously monitor wheel speed sensor electrical plausibility and signal integrity and shall detect open-circuit, short-circuit, stuck, and implausible sensor conditions.
...
The ESC function shall selectively apply wheel brake interventions to mitigate unstable yaw behavior when sensed vehicle motion deviates beyond calibrated stability thresholds.[web:30]
...
Where an EPB is provided, the parking brake shall be actuated by an electric mechanism capable of applying and holding the brake under specified load and gradient conditions.
...
The one-box brake-by-wire system shall interpret driver brake demand using redundant electrical sensing of pedal position, force, or equivalent pedal input variables.[web:18][web:23]
...
The brake-by-wire system shall generate sufficient brake pressure on all controlled wheels to satisfy vehicle-level stopping and stability requirements under normal operating conditions.
...
The one-box system shall detect internal faults and enter a degraded operating state that preserves a defined minimum braking capability and prevents hazardous unintended braking.

[web:16][web:23][web:25]
...
Faults that affect braking performance, safety functions, or parking brake availability shall be communicated to the driver through tell-tales, messages, or equivalent HMI means.[web:7]
...
Safety-relevant braking software and hardware elements shall be protected from interference by lower-integrity functions sharing computational or communication resources.[web:17]
...
The braking system shall support authenticated and validated brake requests from ADAS functions such as AEB when those features are part of the vehicle item definition.[web:12]
...
Topic Legacy Hydraulic Brake-by-Wire One-Box Brake demand Mechanical pedal plus hydraulic pressure Electrical demand sensing plus electro-hydraulic path generation.[web:13][web:14]

actuation.[web:18][web:23][web:28]

Assistance method Vacuum or hydraulic booster.[web:14]

Integrated electro-hydraulic pressure generation.

[web:15] [web:18][web:21] Hydraulic leakage, booster loss, mechanical Sensor plausibility, ECU faults, power supply robustness, Base failure focus retention, sensor faults in adjunct functions.

[web:25][web:15] pressure generation faults, fail-operational behavior.

[web:16][web:23][web:25] Regenerative coordination Not typically applicable in conventional architecture.[web:6]

Explicit friction/regen blending required in electrified platforms.[web:18][web:28]

Safety complexity Lower E/E coupling in base service brake path.[web:25]

High E/E dependency requiring stronger functional safety architecture.[web:16][web:22]
